...
#     raise RuntimeError("CUDA is not available. Please set Colab runtime to GPU (e.g., A100).")
...
# EPOCHS = 30
# HIDDEN_DIM = 32
# SUBSAMPLE_FACTOR = 10  # Subsample integration time points

# # Helper Functions
# def parse_numeric_string(s):
#     """Parse string representations of numeric arrays."""
#     try:
...
#             self.electrode_data = scaler.fit_transform(self.electrode_data.reshape(-1, NUM_NODES)).reshape(self.electrode_data.shape)
...
# # GNN Function with Memory Module
...
#         )
#         self.dx_head = nn.Linear(hidden_dim, hidden_dim)
#         self.dm_head = nn.Linear(hidden_dim, hidden_dim)
#         self.diffusion_coef = 0.05
#         self.decay_coef = 0.01

#     def forward(self, t, z):
#         batch_size, num_nodes, total_dim = z.shape
#         assert total_dim == 2 * self.hidden_dim
...
#         # Normalize transformed to prevent extreme values
#         transformed = F.normalize(transformed, dim=-1) * self.hidden_dim**0.5
#         neighbor_influence = torch.matmul(self.adj_matrix.float(), transformed)
#         dx_dt = self.dx_head(transformed + self.diffusion_coef * neighbor_influence - self.decay_coef * x)
#         dm_dt = self.dm_head(transformed + self.diffusion_coef * neighbor_influence)
#         dx_dt = torch.clamp(dx_dt, min=-5, max=5)
#         dm_dt = torch.clamp(dm_dt, min=-5, max=5)
#         dz_dt = torch.cat([dx_dt, dm_dt], dim=-1)
#         return dz_dt

# # Neural ODE GNN with Memory
...
#         initial_x = x[:, 0, :, :]  # Use first time step as initial state
#         z0 = self.encoder(initial_x)  # [batch_size, num_nodes, 2*hidden_dim]
#         # Subsample integration time
#         integration_time = torch.arange(0, seq_len, SUBSAMPLE_FACTOR, dtype=torch.float32).to(device)
#         if integration_time[-1] < seq_len - 1:
#             integration_time = torch.cat([integration_time, torch.tensor([seq_len - 1], dtype=torch.float32).to(device)])
#         # Use dopri5 solver as requested
#         solution = odeint(self.ode_func, z0, integration_time, method='dopri5', rtol=1e-3, atol=1e-4)
#         # Interpolate solution to full sequence length
...
#                     # Linear interpolation using searchsorted and lerp
#                     idx = torch.searchsorted(integration_time, full_time, right=True)
...
#                     weights = (full_time - t0) / (t1 - t0 + 1e-8)  # Avoid division by zero
#                     full_solution[:, b, n, d] = v0 + (v1 - v0) * weights.clamp(0, 1)
#         return full_solution  # [seq_len, batch_size, num_nodes, 2*hidden_dim]

# # Main Temporal GNN Model
...
#     def forward(self, x):
#         solution = self.neural_ode_gnn(x)  # [seq_len, batch_size, num_nodes, 2*hidden_dim]
#         pred_x = solution[:, :, :, :self.neural_ode_gnn.hidden_dim]  # [seq_len, batch_size, num_nodes, hidden_dim]
#         pred_observations = self.decoder(pred_x)  # [seq_len, batch_size, num_nodes, in_channels]
#         pred_sequences = pred_observations.permute(1, 0, 2, 3)  # [batch_size, seq_len, num_nodes, in_channels]
#         return pred_sequences, solution

# # Utility Function for Adjacency Matrix
...
#     sample_data = np.array(sample_data).T if sample_data else np.zeros((NUM_NODES, 1))
...
#         model.train()
#         total_loss = 0
#         for sequences, actual_lengths in tqdm(train_loader, desc=f"Epoch {epoch+1}/{EPOCHS}"):
...
#         plt.plot(actual_sequences[sample_idx, :, 0, 0], label='Actual')
#         plt.plot(pred_sequences[sample_idx, :, 0, 0], label='Predicted')
#         plt.title("Neural Activity Dynamics (Sample 0, Node 0)")
#         plt.xlabel("Time")
#         plt.ylabel("Activity")
#         plt.legend()
#         plt.savefig("results/dynamics_sample.png")
#         plt.close()
#         print("Saved dynamics visualization to results/dynamics_sample.png")
...
# # Optimized StableGNNFunc with improved stability for ECoG data
# class StableGNNFunc(nn.Module):
#     def __init__(self, hidden_dim, adj_matrix):
#         super(StableGNNFunc, self).__init__()
#         self.hidden_dim = hidden_dim
...
#             nn.Linear(2 * hidden_dim, hidden_dim),
#             nn.LayerNorm(hidden_dim),
#             nn.SiLU(),  # Smoother activation function
#             nn.Linear(hidden_dim, hidden_dim),
#             nn.LayerNorm(hidden_dim)
#         )
...
#         # Coefficients tuned for ECoG data
#         self.diffusion_coef = 0.02  # Reduced for stability
#         self.decay_coef = 0.03      # Increased decay for stability

#         # Scale factor controls the overall dynamics speed
#         self.scale_factor = 2.0     # Reduced for more stability
...
# # Improved Neural ODE with better stability for dopri5
# class StableNeuralODEGNN(nn.Module):
#     def __init__(self, in_channels, hidden_dim, adj_matrix):
#         super(StableNeuralODEGNN, self).__init__()
#         self.hidden_dim = hidden_dim
...
#         # Deeper encoder for better initial condition
#         self.encoder = nn.Sequential(
#             nn.Linear(in_channels, hidden_dim),
#             nn.LayerNorm(hidden_dim),
#             nn.ReLU(),
...
#         # Using denser time sampling for stability
#         integration_time = torch.linspace(0, seq_len-1, seq_len//SUBSAMPLE_FACTOR + 1, dtype=torch.float32).to(device)

#         # Use dopri5 with careful tolerance settings
#         solution = odeint(
#             self.ode_func,
#             z0,
#             integration_time,
#             method='dopri5',
#             rtol=1e-2,  # Increased for stability
#             atol=1e-3   # Increased for stability
#         )

#         # Linear interpolation to full sequence length
#         full_time = torch.arange(seq_len, dtype=torch.float32).to(device)
#         full_solution = torch.zeros(seq_len, batch_size, num_nodes, 2 * self.hidden_dim, device=device)
...
#         # Perform interpolation for each batch, node, and dimension
#         for b in range(batch_size):
#             for n in range(num_nodes):
#                 for d in range(2 * self.hidden_dim):
#                     t0 = integration_time[idx - 1]
...
# # Enhanced Temporal GNN
# class EnhancedTemporalGNN(nn.Module):
#     def __init__(self, in_channels, hidden_dim, adj_matrix):
#         super(EnhancedTemporalGNN, self).__init__()
#         self.neural_ode_gnn = StableNeuralODEGNN(in_channels, hidden_dim, adj_matrix)

#         # Multi-layer decoder with residual connection
...
# # Create correlation adjacency matrix with progress bar
# def create_correlation_adjacency_matrix(data, valid_indices, n_samples=50000, threshold=0.5):
#     print(f"Computing correlation adjacency matrix across {n_samples} timestamps...")
#     sample_data = []
...
#     # Use AdamW with reduced learning rate and weight decay
...
#     # Generous scheduler for gradual learning rate reduction
...
#     # Save model weights
#     torch.save(model.state_dict(), "results/ecog_ode_model.pt")
#     print("Model saved to results/ecog_ode_model.pt")
...
#         # Plot first sample's first node
#         sample_idx = 0
#         node_idx = 0
#         plt.figure(figsize=(12, 6))
...
#         # Plot additional nodes for comparison
#         for node_idx in [20, 40, 80]:
#             plt.figure(figsize=(12, 6))
#             plt.plot(actual_sequences[sample_idx, :actual_length, node_idx, 0], label='Actual')
#             plt.plot(pred_sequences[sample_idx, :actual_length, node_idx, 0], label='Predicted')
#             plt.title(f"Neural Activity Dynamics (Sample {sample_idx}, Node {node_idx})")
...
#             plt.ylabel("Normalized Activity")
#             plt.legend()
#             plt.savefig(f"results/dynamics_node_{node_idx}.png")
#             plt.close()
...
# # Constants - using original successful settings
# NUM_NODES = 162
# IN_CHANNELS = 1
# MAX_SEQ_LENGTH = 5000
# BATCH_SIZE = 2  # Original batch size
# LEARNING_RATE = 0.0005  # Original learning rate
# EPOCHS = 50
# HIDDEN_DIM = 32
# SUBSAMPLE_FACTOR = 10  # Original subsample factor
...
#             try:
#                 scaler = StandardScaler()
#                 self.electrode_data = scaler.fit_transform(
#                     self.electrode_data.reshape(-1, NUM_NODES)
#                 ).reshape(self.electrode_data.shape)
#             except Exception as e:
...
#         # Multi-layer GNN with LayerNorm for stability (using original architecture)
#         self.gnn_layer = nn.Sequential(
#             nn.Linear(2 * hidden_dim, hidden_dim),
#             nn.LayerNorm(hidden_dim),
#             nn.SiLU(),  # Using original SiLU activation
#             nn.Linear(hidden_dim, hidden_dim),
#             nn.LayerNorm(hidden_dim)
#         )
...
#         # Graph diffusion - ensure adj_matrix is on same device
#         if self.adj_matrix.device != transformed.device:
#             self.adj_matrix = self.adj_matrix.to(transformed.device)
...
#         # Add gradient norm clipping for stability
#         norm = torch.norm(dz_dt, dim=-1, keepdim=True)
#         max_norm = 10.0  # Set a reasonable maximum gradient magnitude
...
#         # Deeper encoder for better initial condition (using original architecture)
#         self.encoder = nn.Sequential(
#             nn.Linear(in_channels, hidden_dim),
#             nn.LayerNorm(hidden_dim),
#             nn.ReLU(),
...
#         # Using original time sampling for stability
#         integration_time = torch.linspace(0, seq_len-1, seq_len//SUBSAMPLE_FACTOR + 1, dtype=torch.float32).to(device)

#         print(f"Starting ODE integration with {len(integration_time)} time points...")

#         try:
#             # Use dopri5 with careful tolerance settings (on GPU as in original code)
...
#                     z0,
#                     integration_time,
#                     method='dopri5',  # Original solver
#                     rtol=1e-2,        # Original tolerance
#                     atol=1e-3         # Original tolerance
#                 )

#             print(f"ODE integration complete. Solution shape: {solution.shape}")

#             # Handle any NaN values in the solution
...
#             # Linear interpolation to full sequence length - using original approach
#             full_time = torch.arange(seq_len, dtype=torch.float32).to(device)
#             full_solution = torch.zeros(seq_len, batch_size, num_nodes, 2 * self.hidden_dim, device=device)
...
#             # Perform interpolation for each batch, node, and dimension - with error handling
#             for b in range(batch_size):
#                 for n in range(num_nodes):
#                     for d in range(2 * self.hidden_dim):
#                         t0 = integration_time[idx - 1]
...
#         except Exception as e:
#             logger.error(f"Error in ODE solution: {str(e)}")
#             print("Falling back to simpler integration method...")
...
#             except Exception as e:
#                 logger.error(f"Fallback integration also failed: {str(e)}")
...
#                     state = state + self.ode_func(torch.tensor(t, device=device), state) * 1.0  # Simple Euler step
#                     full_solution[t] = state

#                 return full_solution

# # Enhanced Temporal GNN - using original architecture
# class EnhancedTemporalGNN(nn.Module):
#     def __init__(self, in_channels, hidden_dim, adj_matrix):
#         super(EnhancedTemporalGNN, self).__init__()
#         self.neural_ode_gnn = StableNeuralODEGNN(in_channels, hidden_dim, adj_matrix)

#         # Multi-layer decoder with residual connection (using original architecture)
#         self.decoder = nn.Sequential(
#             nn.Linear(hidden_dim, hidden_dim),
#             nn.LayerNorm(hidden_dim),
#             nn.ReLU(),
...
#         # Use AdamW with reduced learning rate and weight decay (original settings)
#         optimizer = torch.optim.AdamW(
#             model.parameters(),
#             lr=LEARNING_RATE,
#             weight_decay=1e-6,
...
#         # Generous scheduler for gradual learning rate reduction (original settings)
#         scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
#             optimizer,
#             mode='min',
#             factor=0.7,
...
#                     # Very conservative gradient clipping (original value)
#                     torch.nn.utils.clip_grad_norm_(model.parameters(), max_norm=0.25)
...
#             # Try multiple nodes for better visualization
